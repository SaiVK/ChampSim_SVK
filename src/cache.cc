                ...
                if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss
                    ...
                    if (lower_level)
                        ...
                        if (cache_type == IS_STLB) {
                            ...
                            uint16_t pcid = (RQ.entry[index].asid[1] << 9) | RQ.entry[index].asid[0];
                            uint64_t pa = va_to_pa(read_cpu, RQ.entry[index].full_addr, pcid, RQ.entry[index].type);
...
//for get set no need to include pcid
...
//for get way we should include pcid
uint32_t CACHE::get_way(uint64_t address, uint32_t set,uint16_t pcid)
{
    if(cache_type == IS_STLB || cache_type == IS_ITLB || cache_type == IS_DTLB || cache_type == IS_PML4C || cache_type == IS_PDPTC || cache_type == IS_PDC){
        ...
        for (uint32_t way=0; way<NUM_WAY; way++) {
            if (block[set][way].valid && (block[set][way].tag == address) && block[set][way].pcid == pcid) 
                return way;
        }
    }
    else{
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    if (cache_type == IS_PML4C) {
        if (packet->data == 0)
            assert(0);
    }

    if (cache_type == IS_PDPTC) {
        if (packet->data == 0)
            assert(0);
    }

    if (cache_type == IS_PDC) {
        if (packet->data == 0)
            assert(0);
    }
#endif
    ...
    //setting pcid
    block[set][way].pcid = (packet->asid[1] << 9) | packet->asid[0];
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    if(cache_type == IS_STLB || cache_type == IS_ITLB || cache_type == IS_DTLB || cache_type == IS_PML4C || cache_type == IS_PDPTC || cache_type == IS_PDC){
        ...
        uint64_t current_pcid = (packet->asid[1] << 9) | packet->asid[0];

#ifdef SANITY_CHECK
        if(current_pcid == UINT16_MAX || current_pcid == ((UINT8_MAX << 8) + UINT8_MAX)){
            assert(0);
        }
#endif          
        // hit
        for (uint32_t way=0; way<NUM_WAY; way++) {
            if (block[set][way].valid && (block[set][way].tag == packet->address ) && (block[set][way].pcid == current_pcid)) {
...
int CACHE::invalidate_entry(uint64_t inval_addr,uint16_t pcid)
{
    ...
    if(cache_type == IS_ITLB || cache_type == IS_DTLB || cache_type == IS_STLB || cache_type == IS_PML4C || cache_type == IS_PDPTC || cache_type == IS_PDC ){
        ...
        // invalidate      
        for (uint32_t way=0; way<NUM_WAY; way++) {
            if (block[set][way].valid && (block[set][way].tag == inval_addr) && (block[set][way].pcid == pcid)) {
