...
extern map<uint16_t,uint64_t> pcid_pml4_map; //ampping between pcid and pml4 base address
extern map<uint64_t,pair<uint16_t,uint64_t>> inverse_table; //PFN,pcid,VPN
extern map<uint64_t,pair<bool,bool>> nru_state; //nru maintained for the corresponding dram pages,(PFN,R,M)
extern vector<uint64_t> nru_pages; //stores only the PFN where R and M bit are 0
extern uint8_t page_swap; //swap occured or not 
...
uint64_t rotl64 (uint64_t n, unsigned int c),
         ...
         va_to_pa(uint32_t cpu, uint64_t va_addr, uint16_t pcid, uint8_t type);
